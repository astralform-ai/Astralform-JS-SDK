...
describe("public export surface", () => {
  ...
  it.each([
    ...
    ["custom-events.ts", 20],
    ["types.ts", 60],
    // #78 added the last three: `ConversationJob` shipped as the payload of
    // two StreamManager events while being unnameable from the package, which
    // is exactly the failure this guard exists to catch — it just was not
    // looking at `client.ts`. The others were clean already and are here so
    // they cannot stop being clean silently.
    //
    ...
    // the @internal opt-out" below, so these are belt-and-braces. Referenced
    // by name rather than by position, which drifts as tests are added.
    // Setting a floor just under the current count instead turns the
    // documented `@internal` opt-out into a spurious failure: marking one type
    // internal drops the count and fails a guard that is working correctly.
    // Hence floors well under each file's count, not one below it.
    //
