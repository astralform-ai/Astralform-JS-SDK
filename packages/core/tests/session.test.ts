...
describe("ChatSession", () => {
  ...
  it("send passes imageMode to the request", async () => {
    // Without image_mode on the wire the backend never attaches the image tool,
    // so a composer that "turned image mode on" would send an ordinary turn and
    // the agent would simply have no way to generate anything.
    let capturedBody: string | undefined;
    const sseData = [
      'event: message_start\ndata: {"type":"message_start","turn_id":"t1","model":"m","job_id":"job-1","seq":0,"ts":0}\n',
      "",
    ...
    await session.connect();
    await session.send("Draw a cat", { imageMode: true });

    const body = JSON.parse(capturedBody!);
    expect(body.image_mode).toBe(true);
  });

  it("send omits image_mode on an ordinary turn", async () => {
    // Per-message and off by default: generating costs the developer real money,
    // so an unset flag must not arrive as anything the backend reads as on.
    let capturedBody: string | undefined;
    const sseData = [
      'event: message_start\ndata: {"type":"message_start","turn_id":"t1","model":"m","job_id":"job-1","seq":0,"ts":0}\n',
      "",
  ...
  it("send passes goal (goal mode) to the request", async () => {
    ...
    await session.connect();
    await session.send("Build me a market research report", {
      goal: "Build me a market research report",
    });

    expect(capturedBody).toBeDefined();
    const body = JSON.parse(capturedBody!);
    expect(body.goal).toBe("Build me a market research report");
  });
