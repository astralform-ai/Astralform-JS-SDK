...
- **`client.getToolOutput(conversationId, callId, jobId?)`** — the body behind a stub. **Pass the stub's `job_id`.** `/events?job_id=X` deliberately returns jobs that regeneration has replaced, since reading a superseded version is the whole purpose of that parameter, while this route excludes them unless scoped to a job. An unscoped fetch therefore 404s on exactly the pills a version-switched read is displaying — and restore fetches events per job, always passing `job_id`, so that is the normal path rather than an edge case.
...
- **The Astralform version floors named releases that do not exist.** Every "requires Astralform >= X" in the code-mode docs was a forward guess at the release number rather than the release that actually shipped the feature. The phase-1 fields (`Conversation.repository`, `SendOptions.repository` / `ChatStreamRequest.repository`, `AgentInfo.mode`, `client.code.projects.*`) said **0.70.0** and shipped in **0.69.46** (astralform-ai/Astralform#1060); `codeProjectsEnabled` and the optional-repository behaviour said **0.71.0** and shipped in **0.69.50** (#1082). Neither 0.70.0 nor 0.71.0 has ever been tagged, so a reader checking their backend against the documented floor got no answer at all. Docs only — no behaviour, no types.
...
Requires Astralform >= 0.69.50 for `codeProjectsEnabled` and for the optional-repository behaviour; against an older backend the field is absent and a code-mode agent still refuses a first turn with no repository.

## 7.4.1

### Fixed

- **`StreamManager.send` now forwards `repository`.** 7.4.0 added the option to `ChatSession.send`, but the manager has its own `SendOptions` and its forward to the session is a second allowlist — so a client sending through `StreamManager` (which is what the chat app does) could set the option and have it silently dropped before the request was built. Same write-once semantics: the first turn binds the task, a later turn may omit it or repeat the same value, and a different value is refused rather than ignored.

## 7.4.0

### Added

- **Code mode** — the client half of Projects and Tasks (astralform-ai/Astralform#1060, #1070). An agent can be put in code mode, where every task is bound to one GitHub repository and its runs carry a token scoped to that repository; this release carries the fields a client needs to show it. `AgentInfo.mode` (`"chat" | "code"`) is what a client branches on — it is absent from an Astralform older than 0.69.46, which means chat, the server's own default. `Conversation.repository` names the project a task belongs to (`null` for an ordinary conversation), and `getConversations(limit, offset, { repository })` narrows the list to one project's tasks. `SendOptions.repository` / `ChatStreamRequest.repository` name the project when a task is started: the binding is write-once server-side: the first turn binds the task, a later turn may omit it or repeat the same value, and a different value is refused rather than ignored — a first turn without it on a code-mode agent is refused, because the run needs a repository before it can hold a credential scoped to one.
- **`client.code.projects`** — `list()`, `available()`, `add(repoFullName)` and `remove(owner, repo)` over `/v1/code/projects*`. A project list is per app user within the agent: the developer connects the workspace's GitHub account, each user curates their own list from what that connection covers, and every method 404s on a chat-mode agent so the surface is invisible rather than empty. Read `available().state` before its list — `ok`, `not_installed` and `unavailable` are three different empty pickers, and only the first means "you have added everything". `totalCount` is the installations' raw total BEFORE already-added projects are subtracted, so it is not the list's length. A repository the workspace cannot reach is refused the same way one owned by someone else is, deliberately: the difference would say which organisations use Astralform. New exported types `CodeProject`, `AvailableRepository`, `AvailableRepositories`. `AgentInfo.mode` is a property of the WORKSPACE: `GET /v1/agents` selects the workspace row itself and returns exactly one entry, so read `agents[0].mode`; the workspace picker (`listAgents`) does not carry it.

Requires Astralform >= 0.69.46 for every field above; against an older backend they are simply absent (`mode`, `repository`) or 404 (`code.projects.*`).

## 7.3.0

### Added

- **Voice input** — the client half of the per-agent voice pipeline (astralform-ai/Astralform#1057): `getVoiceConfig()` (`GET /v1/voice/config` — modes, defaults and vocabulary, never the provider), `transcribeVoice(blob, options)` (`POST /v1/voice/transcriptions`, one multipart form with the recording, the caller's vocabulary and a language hint), and `streamVoicePolish(request, { signal })` (`POST /v1/voice/polish`, an async generator of typed `delta` / `done` / `error` frames; `done.text` is authoritative). Server-reported failures arrive as `error` frames, but the iteration itself rejects on the paths every SSE consumer already handles: aborting the signal cancels the model call upstream and rejects with `StreamAbortedError`; a non-2xx open rejects with `AuthenticationError`, `RateLimitError` or `ServerError`. `transcribeVoice` takes an optional `signal` too — it runs outside the 30 s default deadline because an upload can be as long as the recording, so the signal is the only way to give up on a stalled one (it rejects with the abort reason: the runtime's `AbortError`, or what was passed to `abort(reason)`). `parseVoicePolishFrame` is exported for consumers that read the raw stream. New types `VoiceConfig`, `VoiceTranscript`, `VoiceTranscribeOptions`, `VoicePolishRequest`, `VoicePolishEvent`, `VoicePolishMode`, `VoiceLLMMode`, plus `VOICE_POLISH_MODES` and the guards `isVoicePolishMode` / `isVoiceLLMMode` — `VoiceConfig.defaultMode` can be `raw`, which `streamVoicePolish` refuses, and a mode the server adds later falls back to `structured` rather than reaching a typed `switch`. Vocabulary: the server holds the project list and merges it in first, so the `hotwords` a client sends are only the user's own words.
