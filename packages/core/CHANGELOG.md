...
A page request in flight when `connect()` re-seeds the list is discarded via a paging generation token. Applying it would append the wrong rows on top of the fresh page-1 state — leaving a page-sized hole — and leave the offset too high, so every later request re-fetched the same offset and paging never advanced again. The invalidation is tied to the list actually being re-seeded: a `connect()` whose own list fetch fails leaves the offset intact, and the in-flight page still applies.
